...
gen_root_cert(){
	...
	echo "Before we create the certificate, let's set the Certificate Revocation List (CRL) Distribution Point, which is the point you'll use to distribute the CRL. Other's will use it to verify that a certificatesigned by you is not revoked. WARNING: Once it is set, you cannot change it. NOTE: the crl is currently named 'crl.pem', located in the current directory (./)."
	...
	echo "Now, we will create a self-signed certificate for our CA, in X.509 format, that lasts 10 years, with SHA2-512. This will use your private key, so it will prompt you for the passphrase."
	...
	echo "The certificateis now accessible by anyone to read."
	...
	echo "Done! Your certificateis now ready to be used!"
}